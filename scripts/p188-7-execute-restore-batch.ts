...
import {
  buildP1887RollbackPlanMarkdown,
  executeP1887BatchRestore,
...
async function main() {
  ...
  const testRun = spawnSync(
    ...
    [
      ...
      "src/lib/p188-6-recruiter-restore-batch/__tests__/p188-6-recruiter-restore-batch.test.ts",
    ],
    { cwd: process.cwd(), encoding: "utf8" },
  );

  const preflight = await runP1887Preflight({ priorTestsPassed: testRun.status === 0 });
  await writeFile(
    path.join(ART, "p188-7-batch-preflight.json"),
  ...
  if (prior.allMembers.length !== 60) {
    console.error(
      JSON.stringify(
        { aborted: true, reason: `expected 60 prior members, got ${prior.allMembers.length}` },
        null,
        2,
      ),
    );
  ...
  const cohort = await freezeP1887BatchCohort({
    workflows,
    breezyCandidates,
    p158Events: p158,
    excludeCandidateIds: prior.allMembers.map((m) => m.candidateId),
    size: 50,
  });
  const auth = newP1887Authorization(cohort, "operator:p188.7-prompt");

  await writeFile(
    path.join(ART, "p188-7-frozen-cohort.json"),
    `${JSON.stringify(redactCohortForPublic(cohort), null, 2)}\n`,
  );
  await writeFile(
    path.join(recruitingDataDir(), "p188-7-recruiter-restore-batch-local.json"),
    `${JSON.stringify(
      {
        ...
        authorization: auth,
        cohort,
        priorCanaryCohortId: prior.canary?.cohortId ?? null,
        priorBatch6CohortId: prior.batch6?.cohortId ?? null,
      },
      null,
      2,
    )}\n`,
  ...
  const execution = await executeP1887BatchRestore({ cohort, authorization: auth });
  await writeFile(
    path.join(ART, "p188-7-production-restore-report.json"),
    `${JSON.stringify(
      {
  ...
  await writeFile(
    path.join(ART, "p188-7-rollback-plan.md"),
    buildP1887RollbackPlanMarkdown({ cohort, execution }),
  );
  ...
  const ingestionReport = await runP1887IngestionDurabilityChallenge({
    newBatch: cohort,
    priorMembers: prior.allMembers,
  });
  await writeFile(
    path.join(ART, "p188-7-ingestion-durability-report.json"),
    `${JSON.stringify(
      {
        sourcePhase: P188_7_SOURCE_PHASE,
        ...ingestionReport,
  ...
  if (
    ...
    ingestionReport.priorRestoredPreserved !== 60 ||
  ...
  for (const wf of afterWorkflows) {
    ...
    simulated.push({
      ...
      idempotencyKey: `p188.7-forecast:${wf.candidateId}`,
  ...
  const readiness = {
    ...
    remainingImpossibleUnrecoverable: remainingPreview.totals.insufficientEvidence,
  ...
  await writeFile(
    path.join(ART, "p188-7-readiness-forecast.json"),
    ...
    JSON.stringify(
      {
        ...
        ingestion: {
          ...
          priorRestoredPreserved: ingestionReport.priorRestoredPreserved,
