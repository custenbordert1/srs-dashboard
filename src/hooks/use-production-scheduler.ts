...
const CACHE_KEY = cacheKey(["p167", "production-scheduler"]);

function readCache(): P167ProductionSchedulerReport | null {
  if (typeof window === "undefined") return null;
  return getCachedAllowExpired<P167ProductionSchedulerReport>(CACHE_KEY);
}

export function useProductionScheduler() {
  const initial = readCache();
  const [report, setReport] = useState<P167ProductionSchedulerReport | null>(initial);
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const res = await fetchWithTimeout("/api/recruiting/production-scheduler", {
        cache: "no-store",
        timeoutMs: P161_CLIENT_SECTION_TIMEOUT_MS,
      });
      const parsed = (await res.json()) as {
        report?: P167ProductionSchedulerReport;
        meta?: ExecutiveSnapshotClientMeta;
        error?: string;
      };
      if (!res.ok || !parsed.report) {
        throw new Error(parsed.error ?? `Production scheduler failed (${res.status})`);
      }
      ...
      if (cached) {
        ...
        setError(
          isTimeoutError(err)
            ? timeoutErrorMessage("Production scheduler", P161_CLIENT_SECTION_TIMEOUT_MS)
            : friendlyFetchMessageFromError(err, "dashboard") ?? "Scheduler unavailable",
        );
      } else if (!isIgnorableFetchError(err)) {
        setError(friendlyFetchMessageFromError(err, "dashboard") ?? "Scheduler unavailable");
      }
    } finally {
