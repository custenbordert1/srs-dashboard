...
export function resolveEffectiveExecutionMode(input: {
  row: ScoredCandidateWorkflowRow;
  flags: P71FeatureFlags;
}): PaperworkExecutionMode {
  if (!input.flags.automationEnabled || input.flags.executionMode === "off") return "off";
...
export function buildPilotSummary(flags: P71FeatureFlags): string {
