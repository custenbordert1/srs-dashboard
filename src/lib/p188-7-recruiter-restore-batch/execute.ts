...
import { P188_7_SUB_BATCH_SIZE } from "@/lib/p188-7-recruiter-restore-batch/types";

export async function executeP1887BatchRestore(input: {
  ...
  for (let i = 0; i < members.length; i += P188_7_SUB_BATCH_SIZE) {
    const slice = members.slice(i, i + P188_7_SUB_BATCH_SIZE);
    const result = await executeP1885CanaryRestore({
      cohort: { ...input.cohort, members: slice, size: slice.length },
