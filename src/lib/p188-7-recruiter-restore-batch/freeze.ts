...
import {
  P188_7_AUTH_EXPIRATION_HOURS,
  P188_7_BATCH_SIZE,
} from "@/lib/p188-7-recruiter-restore-batch/types";

export async function freezeP1887BatchCohort(input: {
  ...
  const size = input.size ?? P188_7_BATCH_SIZE;
  const frozenAt = new Date(nowMs).toISOString();
  const expiresAt = new Date(nowMs + P188_7_AUTH_EXPIRATION_HOURS * 3600_000).toISOString();
  ...
  for (const item of ordered) {
    ...
    members.push({
      ...
      rollbackReference: `rollback:${item.candidateId}:pre-p188.7:${wf.recruiterOwnershipVersion ?? 0}`,
  ...
  if (selected.length !== size) {
    throw new Error(
      `Unable to freeze P188.7 batch of ${size}; only ${selected.length} eligible remaining`,
    );
  }

  const cohortId = `p188.7-batch-${createHash("sha256")
...
export function newP1887Authorization(frozen: P1885FrozenCohort, actor: string) {
  return {
    actor,
    authorizedAt: new Date().toISOString(),
    cohortId: frozen.cohortId,
    fingerprint: frozen.fingerprint,
    maxRecruiterWrites: P188_7_BATCH_SIZE,
    maxLedgerEvents: P188_7_BATCH_SIZE,
    expiresAt: frozen.expiresAt,
    scope: "fifty_candidate_recruiter_ownership_restore_only" as const,
    authorizationToken: `p188.7:${frozen.fingerprint}:${randomUUID().slice(0, 8)}`,
  };
}
